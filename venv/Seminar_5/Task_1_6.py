...
# Таблицу создайте в виде однострочного генератора, где каждый элемент генератора - отдельный пример таблицы умножания.
...
table_gen = (f'{first :> 6} x {second :> 2} = {first * second :> 3}'
             for first in range(MIN, MAX + 1) for second in range(MIN, MAX + 1))
print(*table_gen, sep='\t')